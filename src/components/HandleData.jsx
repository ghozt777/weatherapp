...
export const Handler = ({ setWeatherData , setUnits}) => {
  ...
  useEffect(async () => {
   if(apiK!=''){
    ...
    if(data.cod==200){
    const date_sunrise = await new Date(data.sys.sunrise * 1000);
    const date_sunset = await new Date(data.sys.sunset * 1000);
  ...
  }, [loginCredentials.location, loginCredentials.units,loginCredentials.apiKey]);
